...
def download_pdf(arxiv_id, output_folder, cache_folder):
    cache_path = os.path.join(cache_folder, f"{arxiv_id}.pdf")
...
def main():
    ...
    with tqdm(total=len(papers), desc="Downloading PDFs", unit="pdf") as pbar:
        for paper in papers:
            if os.path.exists(os.path.join(cache_folder, f"{paper['arxiv_id']}.pdf")):
                cached_downloads += 1
            if download_pdf(paper['arxiv_id'], output_folder, cache_folder):
                successful_downloads += 1
            pbar.update(1)
            time.sleep(1)  # Be nice to the arXiv servers
    
    print(f"\nSuccessfully downloaded {successful_downloads} out of {len(papers)} PDFs")
    print(f"Used cache for {cached_downloads} PDFs")
    ...
    print(f"PDF cache is stored in: {cache_folder}")
